...
f1_noise = f_noise['X'][:] #Array of I-Q data (noise)
f2_noise = f_noise['Y'][:] #Array of Modulation modes (noise)
f3_noise = f_noise['Z'][:] #Array of SNR data (noise)
...
for modu in range(24):
    ...
    filename = classes[modu] + '_UMAP_Plot.png'
    
    plt.setp(ax, xticks=[], yticks=[])
    plt.colorbar(im)
    plt.title('Radio ML 2018 Dataset '+classes[modu]+' UMAP')
    plt.show()
